...
exports.login = async (req, res, next) => {
  try {
    const user = await repository.findAsync(
      req.body.email,
    ...
    res
      .status(400)
      .send([
        { message: "Falha ao tentar realizar login do membro.", data: error },
      ]);
  }
};
...
exports.findById = async (req, res, next) => {
  try {
    ...
    res
      .status(400)
      .send([{ message: "Falha consultar usuário.", data: error }]);
  }
};
...
exports.post = async (req, res, next) => {
  try {
    ...
    contract.hasMaxLen(
      req.body.name,
      255,
      "O nome deve conter no máximo 255 caracteres. "
    );
    ...
    contract.hasMinLen(
      req.body.email,
      3,
      "O e-mail deve conter pelo menos 3 caracteres. "
    );
    contract.hasMaxLen(
      req.body.email,
      128,
      "O e-mail deve conter no máximo 128 caracteres. "
    );
    ...
    res
      .status(400)
      .send([{ message: "Falha ao registrar membro.", data: error }]);
  }
};
...
exports.put = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "Dados do membro atualizados com sucesso!" });
  } catch (error) {
    res
      .status(400)
      .send([{ message: "Falha ao atualizar dados do membro.", data: error }]);
  }
};

exports.findByEmail = async (req, res, next) => {
  try {
    const user = await repository.findByEmailAsync(req.params.email);
    ...
    res
      .status(400)
      .send([
        { message: "Falha ao consultar usuário por e-mail.", data: error },
      ]);
  }
};
